      ...
      <div class='bg-white text-h6 q-pa-md sc-design'>
        ...
        <div class='row full-width justify-center'>
          <span class='col' style='max-width: 500px'>
            <q-input v-model='queryParams.name'
                     class='q-ma-md'
                     dense
                     outlined
                     placeholder='请输入搜索图书名称'
                     square
            >
...
import { bookSearch } from '../../api/cms'
...
export default {
  ...
  methods: {
    selectItem(item) {
      window.open('/book/' + item.id, '_blank')
      // this.$router.push({
      //   path: '/book/' + item.id,
      //   params: {
      //     bookId: item.id
      ...
      if (this.queryParams.hasNextPage) {
        setTimeout(() => {
          if (this.items) {
            ...
            bookSearch(this.queryParams).then(response => {
