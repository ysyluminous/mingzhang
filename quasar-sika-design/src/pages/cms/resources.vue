              ...
              <q-card class='no-border-radius' flat>
                ...
                <q-infinite-scroll :offset='600' @load='onLoad'>
                  <div class='row q-pa-sm'>
                    <div v-for='(item, index) in resource'
                      ...
                      <sc-shadow>
                        <q-card
                          ...
                          flat
                          style='border: 1px solid lightskyblue' @click='selectItem(item)'
                        >
                          <q-item>
...
import { resourceSearch } from '../../api/cms'
...
export default {
  ...
  methods: {
    selectItem(item) {
      window.open('/rec/' + item.id, '_blank')
      // this.$router.push({
      //   path: '/rec/' + item.id,
      //   params: {
      //     resourceId: item.id
      ...
      if (this.queryParams.hasNextPage) {
        setTimeout(() => {
          if (this.items) {
            ...
            console.log('查询资源列表2')
            resourceSearch(this.queryParams).then(response => {
              const resourcesTemp = response.data.list
              this.queryParams.hasNextPage = response.data.hasNextPage
              resourcesTemp.forEach(item => {
    ...
    getResource: function() {
      resourceSearch(this.queryParams).then(response => {
        this.resource = response.data.list
        console.log('查询资源列表')
        this.queryParams.hasNextPage = response.data.hasNextPage
